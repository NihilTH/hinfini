...
export default function Discover() {
  ...
  const load = () => {
    setLoading(true);
    api.get("/products/random", { params: { limit: 100 } }).then(({ data }) => { setItems(data); setLoading(false); });
  };
  useEffect(load, []);
  ...
  return (
    <div data-testid="discover-page" className="max-w-[1400px] mx-auto px-6 lg:px-12 py-16">
      <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
        ...
        <button onClick={load} data-testid="discover-shuffle" className="btn-outline"><Shuffle size={18} /> {t("disc.shuffle")}</button>
      </div>
