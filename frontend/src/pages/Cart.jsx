  ...
  if (items.length === 0) {
    return (
      <div data-testid="cart-empty" className="max-w-2xl mx-auto px-6 py-32 text-center">
        ...
        <Link to="/shop" data-testid="cart-browse-btn" className="btn-primary mt-8 focus-ring">{t("cart.browse")}</Link>
      </div>
    );
  }
  const shipping = shippingFee(subtotal, "home");
  return (
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        ...
        <aside className="bg-[#24221E] border border-[#3d3835] p-8 h-fit" aria-label={t("co.summary")}>
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span>{t("cart.shipping")}</span><span data-testid="cart-shipping">{shipping === 0 ? t("cart.free") : formatPrice(shipping)}</span></div>
            {subtotal < config.free_shipping_from && <div className="text-xs text-[#B8AE95]">{t("cart.freeMsg", { n: new Intl.NumberFormat("hu-HU").format(config.free_shipping_from - subtotal) })}</div>}
          ...
          <Link to="/checkout" data-testid="cart-checkout-btn" className="btn-primary w-full justify-center mt-8 focus-ring">{t("cart.checkout")} <ArrowRight size={16} /></Link>
        </aside>
      </div>
    </div>
  );
